...
- **不勾（默认）**：`/v1/models` 只返回免费模型，请求付费模型直接 403，不会消耗号池的 Premium 额度（注意：`deepseek-v4-flash` 从 2026-08-18 起也算付费了）
...
**列表实不实用**：光有上游模型表不代表你的号现在真能用它。控制台的「实测状态」列合并了三个信号：
...
npm test                # 48 项单测 + 51 项集成测试（会自己起一个临时服务）
```
...
**模型列表里有些模型根本调不通？**
上游模型表只说"官方有这个模型"，不代表你的号现在能用。真实调用失败两次（且是模型本身被拒，不是额度或 token 问题）之后，那个模型会被标成「实测不可用」并默认从 `/v1/models` 里藏掉。想手动重来一遍就点「模型 → 清空实测状态」。

**为什么 Flash 也变成"付费"了？**
上游 2026-08-18 把 `deepseek-v4-flash` 挪进了 premium 池（官方注释写明是临时措施，因为它成了免费模式最大的成本项），而且 DeepSeek 家族另有"每天 1 次"的天花板。现在真正不限量的是 `mimo/mimo-v2.5`。这不是分类错了，是上游改了 —— 控制台「模型 → 列表怎么来的」里能看到当前表的生成时间和真实额度数字。
